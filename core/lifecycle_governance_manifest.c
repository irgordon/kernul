 ...
 *
 * Descriptive, inert governance manifest directory for post-governance metadata surfaces.
 */
...
const lifecycle_governance_manifest_entry_t lifecycle_governance_manifest[] = {
    { .phase_number = 41U, .surface_name = "Governance index surface", .surface_role = SURFACE_ROLE_INDEX },
    {
        .phase_number = 42U,
    ...
    { .phase_number = 44U, .surface_name = "Governance schema surface", .surface_role = SURFACE_ROLE_SCHEMA },
};
...
_Static_assert((sizeof(lifecycle_governance_manifest_surface_role_vocabulary)
                / sizeof(lifecycle_governance_manifest_surface_role_vocabulary[0]))
                   == LIFECYCLE_GOVERNANCE_MANIFEST_EXPECTED_COUNT,
               "governance manifest role vocabulary must match canonical phase range 41-44");
