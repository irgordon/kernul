#ifndef KERNUL_INTERACTIVE_EXECUTION_FAILURE_ACK_GATE_H
#define KERNUL_INTERACTIVE_EXECUTION_FAILURE_ACK_GATE_H

/*
 * include/kernul/interactive_execution_failure_ack_gate.h
 *
 * Kernel-internal failure acknowledgment gate policy contract.
 * Phase 12, Task 1 boundary.
 *
 * This header defines one externally triggered reaction only:
 * publish a session-owned failure acknowledgment gate when the published
 * execution outcome view kind is FAILED.
 *
 ...
 *
 * Gate semantics:
 *   interactive_execution_failure_ack_publish_if_failed() is explicit caller
 *   triggered publication only. No background evaluation, polling, deferred
 *   work, callbacks, or asynchronous behavior is introduced.
 *   Unpublished outcome is a no-op.
 *   COMPLETED outcome is a no-op by intentional asymmetry.
 *   FAILED outcome publishes the session-owned failure ack gate with release
 *   semantics.
 *   Publication is one-shot, monotonic, and idempotent for session lifetime.
 *
 * Acquire semantics:
 *   interactive_execution_failure_ack_acquire() performs explicit acquire
 *   visibility on the failure ack publication marker and returns the stable
...
#endif /* KERNUL_INTERACTIVE_EXECUTION_FAILURE_ACK_GATE_H */
